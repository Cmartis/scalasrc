...
object MyModule {
  ...
  def isSorted(as: Array[Int], op: (Int, Int) => Boolean): Boolean = {
  ...
  private def formatFindFirst[A, B](ss: Array[A], key: B) = {
   ...
   private def formatIsSorted (ss: Array[Int]) = {
      val msg = "The statement that the array [%s] is sorted is %b"
      msg.format(ss.deep.mkString(","), isSorted(ss, gt))
    }
  ...
  //println(formatFindFirst(Array("this", "is", "a", "test"), "test"))
   ...
   println(formatIsSorted(Array(1, 8, 5, 6)))
